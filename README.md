...
The camera follows the center of the configured dice with the same bounded catch-up behavior during a drag and after a Space throw. Transient auto-zoom reacts both to center lag and to the group's physical spread, with portrait compensation on mobile. In advanced multi-dice rolls it tracks only rerollable dice; locked dice follow that center in a camera-relative side row, remain visible and use sensor colliders so they cannot disturb the new throw. Reset restores all dice, the look target, camera position and zoom state immediately.
